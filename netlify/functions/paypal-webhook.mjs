import { handlePayPalEvent, paypalWebhookId, verifyPayPalWebhook } from './_paypal.mjs';
...
export default async (req) => {
  ...
  try {
    ...
    if (!result.ok && result.error === 'user_not_found') {
      console.warn('[paypal-webhook] no matching user', event.event_type, event?.resource?.id);
    ...
    if (!result.ok) {
      console.error('[paypal-webhook] handler failed', result);
    ...
    console.error('[paypal-webhook] fatal', err?.message || err);
